...
");
        ...
        DB::statement('DROP TRIGGER IF EXISTS tr_validate_creditos_homologacion');
        DB::statement('DROP TRIGGER IF EXISTS tr_validate_creditos_homologacion_update');
...
FOR EACH ROW
BEGIN
    DECLARE homologaciones_json JSON;
    DECLARE i INT DEFAULT 0;
    DECLARE asignatura_origen_id INT;
    DECLARE asignatura_destino_raw JSON;
    ...
    SET homologaciones_json = NEW.homologaciones;
    SET array_length = JSON_LENGTH(homologaciones_json);

    WHILE i < array_length DO
        SET asignatura_origen_id = CAST(JSON_UNQUOTE(JSON_EXTRACT(homologaciones_json, CONCAT(\'$[\', i, \'].asignatura_origen_id\'))) AS UNSIGNED);
        SET asignatura_destino_raw = JSON_EXTRACT(homologaciones_json, CONCAT(\'$[\', i, \'].asignatura_destino_id\'));

        IF asignatura_destino_raw IS NOT NULL AND JSON_TYPE(asignatura_destino_raw) != \'NULL\' THEN

            IF JSON_TYPE(asignatura_destino_raw) = \'STRING\' THEN
                SET asignatura_destino_id = CAST(JSON_UNQUOTE(asignatura_destino_raw) AS UNSIGNED);
            ELSE
                SET asignatura_destino_id = CAST(asignatura_destino_raw AS UNSIGNED);
            END IF;
            ...
            IF creditos_origen IS NULL THEN
                SET mensaje_error = CONCAT(\'La asignatura origen con ID \', asignatura_origen_id, \' no existe\');
                SIGNAL SQLSTATE \'45000\' SET MESSAGE_TEXT = mensaje_error;
            END IF;

            IF creditos_destino IS NULL THEN
                SET mensaje_error = CONCAT(\'La asignatura destino con ID \', asignatura_destino_id, \' no existe\');
                SIGNAL SQLSTATE \'45000\' SET MESSAGE_TEXT = mensaje_error;
            END IF;

            IF creditos_origen < min_creditos THEN
                SET mensaje_error = CONCAT(\'La asignatura origen con ID \', asignatura_origen_id,
                                           \' tiene menos de \', min_creditos, \' créditos mínimos requeridos\');
                SIGNAL SQLSTATE \'45000\' SET MESSAGE_TEXT = mensaje_error;
            END IF;
...
');
        ...
        DB::unprepared('
CREATE TRIGGER tr_validate_creditos_homologacion_update
BEFORE UPDATE ON homologacion_asignaturas
FOR EACH ROW
BEGIN
    DECLARE homologaciones_json JSON;
    DECLARE i INT DEFAULT 0;
