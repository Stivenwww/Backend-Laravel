...
class SolicitudCompletaControllerApi extends Controller
{
    public function update(Request $request)
    {
        ...
        try {
            ...
            // 4. Crear registro de homologación
            try {
                Log::info('Creando registro de homologación');

                // Decodificar las materias para crear la estructura de homologaciones
                $materiasArray = json_decode($materiasJson, true);
                if (!is_array($materiasArray)) {
                    throw new \Exception('Error al procesar materias para homologación');
                }

                // Crear estructura de homologaciones con solo datos de origen
                $homologaciones = [];
                foreach ($materiasArray as $materia) {
                    $homologaciones[] = [
                        'asignatura_origen_id' => $materia['asignatura_id'] ?? 0,
                ...
                $homologacionesJson = json_encode($homologaciones, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);

                // Llamar al procedimiento almacenado para insertar homologación
                DB::statement('CALL InsertarHomologacionAsignatura(?, ?, ?, ?)', [
                    $solicitud->id_solicitud,    // ID de la solicitud recién creada
                    $homologacionesJson,         // JSON con las homologaciones
                    now()->toDateString(),       // Fecha actual
                    null                         // Sin PDF de resolución inicialmente
                ]);

                Log::info('Registro de homologación creado exitosamente');

            } catch (\Exception $e) {
                throw new \Exception('Error al crear registro de homologación: ' . $e->getMessage(), 0, $e);
            }
