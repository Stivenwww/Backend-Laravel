...
{
    try {
        // Primero obtenemos todos los pensums de este programa
        $pensums = DB::table('pensums')
        ...
        if ($pensums->isEmpty()) {
            return response()->json([
                'data' => [],
                'message' => 'No se encontraron pensums para este programa',
                'success' => true
            ], 200);
        }

        // Ahora obtenemos las asignaturas de todos los pensums de este programa
        $asignaturas = Asignatura::whereIn('pensum_id', $pensums)
            ->with(['pensum.programa.facultad', 'pensum.programa.institucion', 'contenidosProgramaticos'])
            ->get();
        ...
        $asignaturasConPrograma = $asignaturas->map(function ($asignatura) {
            ...
            $datosAsignatura = [
                'nombre_programa' => $asignatura->pensum->programa->nombre ?? 'No disponible',
                'id_asignatura' => $asignatura->id_asignatura,
                'pensum_id' => $asignatura->pensum_id,
                'id_programa' => $asignatura->pensum->programa->id_programa ?? null,
                'año_pensum' => $asignatura->pensum->anio ?? null,
                'facultad' => $asignatura->pensum->programa->facultad->nombre ?? 'No disponible',
                'institucion' => $asignatura->pensum->programa->institucion->nombre ?? 'No disponible',
...
{
    try {
        // Primero verificamos que la asignatura pertenezca a un pensum del programa especificado
        $asignatura = Asignatura::where('id_asignatura', $id_asignatura)
            ->with(['pensum.programa.facultad', 'pensum.programa.institucion', 'contenidosProgramaticos'])
            ->whereHas('pensum', function($query) use ($id_programa) {
                $query->where('programa_id', $id_programa);
            })
            ->first();
